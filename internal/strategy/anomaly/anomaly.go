...
// MetricRecord - робастный z-score по скользящему окну (медиана+MAD), общий
// для anomaly и для других модулей (см. internal/stat). Алиас оставлен, чтобы
// не переписывать все места ниже, где тип использовался как MetricRecord.
type MetricRecord = stat.MetricRecord

// NewMetricRecord - см. stat.NewMetricRecord.
func NewMetricRecord(maxLen, minSamples int, scaleFloor float64) *MetricRecord {
	return stat.NewMetricRecord(maxLen, minSamples, scaleFloor)
}
...
// процентах, сильно скошенное распределение - симметризуется через
// stat.LogRatio (см. её комментарий; та же формула, что использует и
// internal/prices/activity.go - для buy-активности нужна ровно та же
// симметризация, а не своя копия).
//
